...
The quickest way to get running is `deploy/setup-local.sh`. It handles everything automatically:

```bash
# First-time setup — checks for Postgres/Milvus, starts Docker if needed,
# creates .env, seeds DB, and starts the API server
./deploy/setup-local.sh

# Subsequent runs — skip reseeding
./deploy/setup-local.sh --skip-seed

# Infrastructure + DB seed only, no API server
./deploy/setup-local.sh --skip-app

# Wipe .env and venv, start completely fresh
./deploy/setup-local.sh --reset
```

**What the script does:**

1. Detects whether PostgreSQL (`:5432`) and Milvus (`:19530`) are already reachable
2. If a service is running (Docker container or native) → uses it as-is
3. If the Docker container exists but is stopped → starts it
4. If neither exists → spins up the full stack via `docker-compose up -d` (Postgres + Milvus + etcd + MinIO)
5. Waits for both services to be ready
6. Creates `.env` with sane defaults if missing, pauses for you to fill in API keys
7. Creates `.venv` and installs `requirements.txt`
8. Seeds the database via `seed_db.py`
9. Starts `uvicorn app.main:app --reload` on port 8000

**Prerequisites:**
- Python 3.9+
- Docker Desktop (only needed if Postgres / Milvus are not already running)

### Running the Cloud Run Worker Locally

The video compilation worker can be built and run locally as a Docker container, matching the Cloud Run environment exactly:

```bash
# Build the worker image and run it against a specific itinerary (legacy mode)
./deploy/setup-local.sh --skip-seed --run-worker \
    --itinerary-id YOUR_ITINERARY_ID \
...
The script rewrites `localhost` → `host.docker.internal` (macOS) or the Docker bridge gateway (Linux) so the container can reach host-bound Postgres and Milvus. GCP application-default credentials are mounted automatically from `~/.config/gcloud/application_default_credentials.json` if present.
...
```bash
# 1. Start services (or use your own Postgres/Milvus)
docker compose up -d
...
cp deploy/.env.gcp.example .env   # then edit with your values
...
| `SECRET_KEY` | JWT signing secret (auto-generated by setup script) |
...
| `VIDEO_COMPILER` | `local` (FFmpeg in-process, default) or `cloudrun` (async Cloud Run Job) |
...
| `PEXELS_API_KEY` | Pexels API key for royalty-free clip fallback |
| `ENABLE_PEXELS_FALLBACK` | `true`/`false` — enable Pexels download when no clip matches (default: `false`) |

### Default Credentials (after seeding)

| Role | Email | Password |
|------|-------|----------|
| Tenant admin | `admin@manike.ai` | `admin123` |
| Super admin | `superadmin@manike.ai` | `superadmin123` |
...
| File | Purpose |
|------|---------|
| `deploy/setup-local.sh` | **Local dev setup** — detects existing services, spins up Docker if needed, seeds DB, starts API, and optionally runs the Cloud Run worker locally |
...
## Work Done
- [x] Local setup script (`deploy/setup-local.sh`) — cross-platform (macOS + Linux), auto-detects services, Docker fallback, Cloud Run worker local testing
